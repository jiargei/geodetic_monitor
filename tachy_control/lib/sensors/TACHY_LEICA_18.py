...
from ..geocom import GEOCOM, TMC, BAP
from ..constants import MAIN
from ..tools import GENERATES


# Code
class LeicaTachy18(TACHY.Tachy):
    """
    ...
    def get_tachymeter_type():
        return {"TACHYMETER_TYPE": "Leica TCA1800 Tachymeter"}
    ...
    def measure(self):
        ...
        :return: UID, SYNCED, CREATED, FACE, TEMPERATURE, PPM_CORR, PRISM_CORR, HORIZONTAL_ANGLE, VERTICAL_ANGLE, SLOPE_DISTANCE, CROSS_INCLINE, LENGTH_INCLINE, EASTING, NORTHING, HEIGHT, REFLECTOR_HEIGHT
        """
        tmp = {'UID': GENERATES.gen_uuid(),
               'SYNCED': 0,
               'CREATED': GENERATES.gen_now(DATE_RETURN=True)}
        tmp.update(self.get_face())  # FACE
        tmp.update(self.tps.CSV_GetIntTemp())  # TEMPERATURE
        tmp.update({"PPM_CORR": 0., "PRISM_CORR": 0.0175})
        self.tps.TMC_DoMeasure()
        tmp.update(self.tps.TMC_GetSimpleMea())
        tmp.update(self.tps.TMC_GetAngle1())
        tmp.update(self.tps.TMC_GetSimpleCoord())
        tmp.update(self.tps.TMC_GetHeight())
